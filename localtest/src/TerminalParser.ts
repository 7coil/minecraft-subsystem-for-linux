...
const bit = {
  ...
  bxor(n: number, bits: number): number {
    throw new Error("Unimplemnted BXOR");
  },
  bor(n: number, bits: number): number {
    throw new Error("Unimplemnted BOR");
  },
  band(n: number, bits: number): number {
...
writeFileSync("output.json", JSON.stringify(TRANSITION_TABLE))
