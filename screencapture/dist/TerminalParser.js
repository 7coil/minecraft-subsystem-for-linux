...
function addToTransitionTable(table, inp, state, action = 0, next) {
    table[(state << 8) | inp] =
        ((action | 0) << 4) | (next === undefined ? state : next);
}
/**
...
var PRINTABLES = range(0x20, 0x7f);
var EXECUTABLES = range(0x00, 0x18);
...
class TerminalParser {
    ...
    parse(s) {
        ...
        var code = 0, transition = 0, error = false, current_state = this.current_state | 0;
        // local buffers
        var printed = -1;
        var dcs = -1;
        var osc = this.osc;
        ...
        for (var i = 0, l = s.length | 0; i < l; ++i) {
            ...
            transition =
                (code < 0xa0
                    ? this.transitions[(current_state << 8) | code | 0] | 0
                    : 16) | 0;
